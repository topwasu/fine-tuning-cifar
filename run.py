...
def get_cifar100(interested_indices, train):
	...
	images = torch.stack([x[0] for x in train_loader if x[1] in interested_indices])
	labels = torch.tensor([interested_indices.index(x[1]) for x in train_loader if x[1] in interested_indices])
...
def train_model(train_images, train_labels, model, num_epochs, lr=1e-3, batch_size=16, verbose=False, use_gpu=False):
...
def test_model(test_images, test_labels, model, batch_size=16, use_gpu=False):
...
def main():
	...
	print(f'Test accuracy = {test_model(test_images, test_labels, resnet, batch_size=64, use_gpu=use_gpu)}')
